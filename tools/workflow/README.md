...
workflow 会按顺序执行质量检查板块中选中的项目，并将结果写入同一个 Excel。组合运行时，
包括术语检查在内的所有项目共用一次主工作簿读取，在内存中完成检查和报告合并后只保存
一次；不会为每个检查重复打开和保存大体积 Excel，也不使用可能产生并发写入冲突的线程池。
配置历史 TB 时，历史 TB 仍作为独立输入读取。
