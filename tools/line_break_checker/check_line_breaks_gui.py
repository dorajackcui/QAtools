...
try:
    from .check_line_breaks import process_excel
except ImportError:
    from check_line_breaks import process_excel


class LineBreakCheckerApp(ttk.Frame):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(self, text="开始检查换行", command=self.run_check).grid(
            row=5, column=0, columnspan=3, sticky="ew"
        )
        ttk.Label(
            self,
            text="规则：逐行比较 source / target 单元格中的真实换行数量；CRLF 按一个换行计算。",
