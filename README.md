...
- 输出增强：结果会给出合并后的 `术语表`（只包含本次检查文本中涉及的术语，并包含保留 mark、无 mark 和术语来源列），以及带原文上下文的 `问题列`；问题表前四列统一为 `行号 / source原文 / target原文 / 问题描述`
...
- 输出方式：生成新的结果 Excel，并新增 `Target中文问题` 工作表；原数据工作表不改动
- 问题表前四列统一为 `行号 / source原文 / target原文 / 问题描述`，第五列列出命中字符
- CLI：
...
`问题处理` 会完整保留各检查的专项信息，例如术语来源、Tag 问题类型、换行数量、同组行号和命中字符。列顺序为 `行号 / source / target / 修改后target / 问题描述 / 检查项`。点击行号可跳到原 target；在黄色的 `修改后target` 列填写最终译文表示需要回填，留空表示忽略。保存后回到 GUI 点击“应用修订”，即可生成 `revised_<原文件名>` 干净修订稿；生成时会核对原 target，并跳过可能发生错位的冲突行。workflow 报告本身不会保留 `术语QA问题` 辅助列。

五个质量检查工具单独运行时，问题工作表中的行号同样可以点击，并会跳转到原数据工作表对应的 target 单元格。

所有工具页面使用统一的极简布局：文件选择单独占一行，工作表、列和开始行紧凑排列，附加设置按功能分组，底部统一提供主操作按钮和自动输出文件名预览。复杂页面支持滚动，窗口缩放时内容会自适应宽度。

### macOS Finder 右键发送到一键质量检查

在 macOS 上可以安装 Finder 快速操作：

```bash
python3 scripts/install_macos_qa_workflow.py
```

安装后，在 Finder 中右键 `.xlsx` 或 `.xlsm` 文件，选择“快速操作”→“QA workflow”。文件会自动填入“一键质量检查”的“输入 Excel”，并自动识别工作表及 Source / Target 列：

- Toolshub 已打开时：复用当前窗口并切到“一键质量检查”。
- Toolshub 未打开时：自动启动 Toolshub 并载入所选文件。

如果 Finder 没有立刻显示该入口，可在“系统设置 → 隐私与安全性 → 扩展 → Finder”中确认“QA workflow”已启用。
