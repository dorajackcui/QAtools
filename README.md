...
`问题处理` 会完整保留各检查的专项信息，例如术语来源、Tag 问题类型、换行数量、同组行号和命中字符。点击行号可跳到原 target；在黄色的 `修改后target` 列填写最终译文，必要时将状态改为 `清空target` 或 `忽略`。保存后回到 GUI 点击“应用修订”，即可生成 `revised_<原文件名>` 干净修订稿；生成时会核对原 target，并跳过可能发生错位的冲突行。workflow 报告本身不会保留 `术语QA问题` 辅助列。
