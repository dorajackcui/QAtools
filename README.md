...
忽略大小写和首尾空格后精确匹配整个表头，例如 `Source` 不会匹配 `SourceFile`。
Source、Target 分别优先匹配自定义别名；没有自定义别名命中时，才使用内置的
`source` / `target`。设置会持久化到当前用户。

同一级匹配到多列或完全未命中时，统一 GUI 会清空对应列并提示“请指定”，需要
手动填写列字母。例如同时存在 `MsgStr` 和 `Source`，且配置了别名 `MsgStr`，
则选择 `MsgStr`；若两列都叫 `MsgStr`，则需要手动指定。
