...
class SubstringConsistencyTests(unittest.TestCase):
    def test_normalized_references_share_versions_and_keep_each_original_offset(self):
        ...
        self.assertIn("参考行 2、3", issues[0][3])
        self.assertIn("Source：【阿童木】", issues[0][3])
        self.assertIn("参考 Target：‘Astro Boy’", issues[0][3])
        self.assertIn("第 3–5 字符", issues[0][3])
        self.assertIn("第 1–3 字符", issues[1][3])
    ...
    def test_reports_parent_with_reference_rows_and_original_position(self):
        ...
        self.assertIn("参考行 2、3", issues[0][3])
        self.assertIn("参考 Target：Save changes", issues[0][3])
        self.assertIn("第 3–6 字符", issues[0][3])
    ...
    def test_duplicate_occurrences_deduplicate_but_all_ancestors_are_checked(self):
        ...
        self.assertIn("2 个原文片段", issues[0][3])
        self.assertIn("参考行 2", issues[0][3])
        self.assertIn("参考行 3", issues[0][3])

    def test_details_are_bounded_but_count_is_complete(self):
        children = [f"词条{index:03}" for index in range(DETAIL_LIMIT + 5)]
        summary, issues, _ = self.check_rows([
        ...
        self.assertEqual(issues[0][3].count("参考行"), DETAIL_LIMIT)
    ...
    def test_matches_brute_force_oracle_on_generated_corpus(self):
        ...
        children = [f"词条{index:03}" for index in range(30)]
        ...
        for issue in issues:
            self.assertIn(f"{expected[issue[0]]} 个原文片段", issue[3])
